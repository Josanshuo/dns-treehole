...
**为什么时间戳要写进内容？** 因为 DNS 不保证同一 RRset 内多条记录的返回顺序，客户端必须自己排序。同时记录的 DNS TTL 固定 60 秒，只管缓存，不是存活时长，所以剩余寿命也得从内容里算。
...
- **生成的码**：有额度（能发几条）和存活时长上限（最长能发多久的帖子，默认 2 分钟），用完即止。存在 `PostGate` DO 的 SQLite 里，扣额度和建记录在同一次排队里完成，记录没建成不扣。

生成、查看、作废都走管理接口，用 `ADMIN_KEY` 保护：

```bash
# 生成 3 个各能发 20 条、最长 1 小时的码
# maxTtl 取存活时长选项之一（秒）：120/300/600/900/1800/3600/7200/18000/43200/86400，省略即 120
...
前端没填邀请码时存活时长下拉只列 2、5 分钟；填完邀请码会显示「还剩 N 条」，下拉改为只列这个码允许的档位（静态码列全）；发布后响应里也带 `left` 和 `maxTtl`（静态码为 `null`）。额度用完返回 403「这个邀请码的额度用完了」，存活时长超了返回 400。
...
- **删除有传播延迟。** 记录的 DNS TTL 固定 60 秒（和帖子寿命无关），所以最坏 60 秒后全网不可见。
- **新帖也有传播延迟。** 解析器会把上一次的答案（通配哨兵或旧记录）缓存到 TTL 用完，Cloudflare 最短 60 秒。实测（2026-09）新帖要 20–60 秒才被 1.1.1.1 / dns.google 看到，所以没有 1 分钟这一档：一分钟的帖子别人基本来不及看。
